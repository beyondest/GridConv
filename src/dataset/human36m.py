...
class Human36M(Dataset):
    def __init__(self, data_path, is_train, exclude_drift_data, prepare_grid):
        ...
        ordered_key = list(filter(lambda x: x[0] in self.subject_list, ordered_key)) # S9, S11 only for test
        sample_step = 1
        for key in tqdm(ordered_key):
            sub, act, fname = key
            fullact = fname.split('.')[0]
            num_f = data_2d[key].shape[0]   # number of frames, (num_f, num_jts * 2) is each data shape
            ...
            for i in range(0, num_f, sample_step):
                p2d_ori = data_2d[key][i].reshape(self.num_jts, 2) # (num_jts, 2), (17, 2) for this human3.6m data
                p3d_ori = data_3d[key]['joint_3d'][i].reshape(self.num_jts, 3) # (num_jts, 3), (17, 3) for this human3.6m data

                p2d = (p2d_ori - 500) / 500.
                p3d = p3d_ori / 1000.
                self.inp.append(p2d)
                self.out.append(p3d)
                self.meta['info'].append({'subject':sub, 'action':fullact, 'camid':fname.split('.')[-1], 'frid':i}) # sub is S9, fullact is Direction 1, camid is 54138969(what next to fullact), frid is frame id
